export class HeuristicStrategy {
	...
	correct(rawInput, expectedSchema) {
		if (Array.isArray(rawInput)) {
			const targetSchema = Array.isArray(expectedSchema)
				? expectedSchema[0]
				: expectedSchema;
		...
		for (const expectedKey of Object.keys(expectedSchema)) {
			...
			if (exactValue !== undefined) {
				correctedOutput[expectedKey] = this._processValue(
					exactValue,
					schemaDefinition,
				);
				continue;
			...
			if (matchedKey) {
				...
				correctedOutput[expectedKey] =
					this._getDefaultValueForType(schemaDefinition);
			}
		}
	...
	_findApproximatedKey(expectedKey, availableKeys) {
		...
		return availableKeys.find(
			(key) => this._normalizeString(key) === normalizedExpected,
		);
	}
